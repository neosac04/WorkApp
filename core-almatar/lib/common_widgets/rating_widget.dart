...
class RatingWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final effectiveTextStyle =
        textStyle ?? Theme.of(context).textTheme.displaySmall!.copyWith(color: Colors.white, fontSize: fontSize.scaledFont(context));
    return Container(
      padding: context.scaledSymmetricDirectionalPadding(horizontal: horizontalPadding, vertical: verticalPadding),
      decoration: BoxDecoration(
        color: Theme.of(context).primaryColor,
        border: Border.all(color: Theme.of(context).primaryColor),
        borderRadius: BorderRadius.circular(AppSizeConstants.size_16.scaledRadius(context)),
      ),
      child: Row(
        mainAxisSize: MainAxisSize.min,
        children: [
          Icon(Icons.star_rate_rounded, size: AppSizeConstants.size_12.scaled(context), color: iconColor),
