...
class _CustomDropdownFieldState extends State<CustomDropdownField> {
  ...
  Widget build(BuildContext context) {
    final textStyle =
        widget.textStyle ??
        Theme.of(context).textTheme.bodyMedium?.copyWith(color: widget.selectedValue.isEmpty ? ColorConstants.darkGrayColor : Colors.black);
            ...
            children: [
              if (widget.prefixIconPath != null)
                SvgGenImage(widget.prefixIconPath!)
                    .svg(colorFilter: widget.selectedValue.isEmpty ? const ColorFilter.mode(ColorConstants.grayColor, BlendMode.srcIn) : null)
                    .paddingDirectionalOnly(context, end: AppSizeConstants.size_6),
              Expanded(child: Text(widget.selectedValue.isNotEmpty ? widget.selectedValue : widget.hintText, style: textStyle)),
              Transform.rotate(
