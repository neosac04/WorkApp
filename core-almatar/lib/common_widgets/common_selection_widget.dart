    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(color: isSelected ? (selectedBorderColor ?? Theme.of(context).primaryColor) : unselectedBorderColor, width: borderWidth),
          borderRadius: BorderRadius.circular(borderRadius),
        ),
        child: Stack(
          children: [
            Padding(padding: padding ?? const EdgeInsets.all(AppSizeConstants.size_14), child: child),
            (isSelection ?? true)
                ? Positioned(
                    ...
                    child: SvgGenImage(
                      isSelected ? Assets.icons.icSelected.path : Assets.icons.icNotSelected.path,
                    ).svg(colorFilter: ColorFilter.mode(isSelected ? Theme.of(context).primaryColor : ColorConstants.grayColor3, BlendMode.srcIn)),
                  )
                : SizedBox.shrink(),
          ],
        ),
