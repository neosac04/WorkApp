...
class ProductCardWidget extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    return Container(
      ...
      decoration: BoxDecoration(
        color: isCancelable ? null : Theme.of(context).scaffoldBackgroundColor,
  ...
  Widget _buildImageStack(BuildContext context, String imageUrl, VoidCallback onFavoritePressed) {
    return Stack(
      children: [
        getNetworkImageViewAPI(context, imageUrl, height: AppSizeConstants.size_165.scaled(context), width: double.infinity, boxFit: BoxFit.cover),
        ...
        if(productData.promotionLabel.isNotEmpty)...[
          PositionedDirectional(
            bottom: 0,
            start: 0,
            child: Container(
              ...
              decoration: BoxDecoration(
                ...
                borderRadius: BorderRadius.only(
                  bottomLeft: Radius.circular(AppSizeConstants.size_8.scaledRadius(context)),
                  topRight: Radius.circular(AppSizeConstants.size_8.scaledRadius(context)),
                ),
              ),
              child: Text(
  ...
  Widget _buildRatingAndFav(VoidCallback onFavoritePressed) {
    return Row(
      ...
      children: [
        if (double.parse(productData.averageRating) > 0) ...[RatingWidget(rating: productData.averageRating)],
        Spacer(),
        if (isFavorite) ...[FavouriteUnFavouriteWidget(isFavorite: productData.isWishListed, onFavoritePressed: onFavoritePressed)],
      ],
    );
  }
  ...
  Widget _buildProductTitle(BuildContext context) {
    return Text(
      productData.name,
      maxLines: 2,
      style: context.textTheme.labelLarge?.copyWith(height: 1.3, overflow: TextOverflow.ellipsis, color: Theme.of(context).primaryColor),
    );
  }
    ...
    return Row(
      children: [
        CurrencyText(
          ...
          style: context.textTheme.displaySmall?.copyWith(color: Theme.of(context).colorScheme.secondary),
        ),
        if (productData.hasDiscount) ...[
          AppSizeConstants.size_4.hBox(context),
          CurrencyText(
            text: productData.originalPrice,
            style: context.textTheme.labelSmall?.copyWith(color: ColorConstants.grayColor),
            isStrikeThrough: true,
          ),
        ],
        if(productData.discountPercentage.isNotEmpty)...[
          AppSizeConstants.size_4.hBox(context),
          Text(
            productData.discountPercentage,
            maxLines: 1,
            style: context.textTheme.displaySmall?.copyWith(color: Theme.of(context).primaryColor, overflow: TextOverflow.ellipsis),
          )
        ]
      ],
    );
