...
class CommonButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final Color effectiveBorderColor = showBorder
        ? WidgetStateColor.resolveWith((states) {
            if (states.contains(WidgetState.disabled)) {
              return (borderColor ?? ColorConstants.primaryColor).withOpacity(0.5);
            }
            return (borderColor ?? ColorConstants.primaryColor);
          })
        : Colors.transparent;

   /* final Color effectiveTextColor = showBorder
        ? WidgetStateColor.resolveWith((states) {
            if (states.contains(WidgetState.disabled)) {
    ...
    final buttonStyle =
        ElevatedButton.styleFrom(
          ...
          backgroundColor: WidgetStateProperty.resolveWith<Color>((states) {
            if (states.contains(WidgetState.disabled)) {
              return disabledBackgroundColor ?? backgroundColor.withOpacity(0.5);
            }
    ...
    if (showBorder) {
      button = Stack(
        children: [
          ...
          SizedBox(height: height.scaledHeight(context), width: width.scaledWidth(context), child: button),
        ],
      );
    }
  ...
  Color get effectiveTextColor {
    if (!showBorder) return Colors.white;
    if (!isClick) return ColorConstants.primaryColor.withOpacity(0.5);
    return ColorConstants.primaryColor;
  }
}
