...
class _FoodCartProductItemState extends State<FoodCartProductItem> {
  ...
  double getPrice() {
    /*if(widget.product.modifiers.isNotEmpty){
      return double.parse((widget.product.modifiers
          .map((modifier) => double.parse(modifier.currentPrice))
          .reduce((a, b) => a + b) + double.parse(widget.product.price)).toStringAsFixed(2));
    }*/

    return double.parse(widget.product.originalItemTotal);
  }

  @override
  Widget build(BuildContext context) {
    String price = (getPrice() * localQty).toString();

    return Container(
      clipBehavior: Clip.hardEdge,
      padding: context.scaledUniformDirectionalPadding(6),
            ...
            children: [
              ClipRRect(
                ...
                child: getNetworkImageViewAPI(
                  ...
                  width: 95.scaledWidth(context),
                  height: 100.scaledHeight(context),
                ),
              ),
              8.hBox(context),
              Expanded(
                child: Column(
                  ...
                  children: [
                    Text(
                      widget.product.name,
                      style: context.textTheme.labelLarge?.copyWith(color: Theme.of(context).primaryColor),
                      maxLines: 2,
                    ...
                    Row(
                      children: [
                        CurrencyText(text: price, style: context.textTheme.displaySmall),
                        4.hBox(context),
