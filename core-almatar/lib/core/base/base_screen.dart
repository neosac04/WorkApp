import 'package:core/core/base/base_notifier.dart'
    show BaseNotifier, BaseProvider;
...
abstract class BaseScreen<T extends Enum, S extends BaseState<T>>
...
abstract class BaseScreenState<
  ...
  VM extends BaseNotifier<S>,
  W extends BaseScreen<T, S>
>
  ...
  /// Called when internet connection is restored after being disconnected.
  /// Override this to retry API calls when internet comes back.
  void onInternetRestored() {}

  ErrorHandlingConfig<T> _getDefaultErrorConfig() {
    return ErrorHandlingConfig<T>(
      showGlobalErrors: true,
      autoMarkErrorsAsShown: true,
  ...
  Widget build(BuildContext context) {
    ...
    // Listen for connectivity changes - retry APIs when internet comes back
    ref.listen<AsyncValue<bool>>(connectivityStreamProvider, (previous, next) {
      final wasDisconnected = previous?.value == false;
      final isNowConnected = next.value == true;

      // Internet just came back - trigger retry
      if (wasDisconnected && isNowConnected) {
        MyLogger.info("🌐 Internet restored - triggering retry");
        onInternetRestored();
      }
    });
    ...
    ref.listen<S>(provider, (previous, next) {
      ...
      CommonErrorListener.handleGlobalError<T, S>(
        context: context,
        next: next,
        previous: previous,
        config: effectiveConfig,
      ...
      CommonErrorListener.handleLoadingDialog<T, S>(
        context: context,
        next: next,
        config: effectiveConfig,
        previous: previous,
  ...
  Widget buildScreenWithErrorHandling(BuildContext context) {
    ...
    MyLogger.warning(" 🌐 Connectivity status: $isConnected");
    if (!isConnected) {
