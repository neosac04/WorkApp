import 'package:core/core/base/base_notifier.dart'
    show BaseProviderFamily, BaseFamilyNotifier;
...
abstract class BaseScreenFamily<T extends Enum, S extends BaseState<T>, P>
...
>
  ...
  NotifierProvider<VM, S> providerFamily(P params);
  ...
  Widget build(BuildContext context) {
    ...
    ref.listen<S>(providerFamily(widget.params), (previous, next) {
      ...
      if (effectiveConfig.autoMarkErrorsAsShown) {
        CommonErrorListener.markErrorsAsShown(
          providerFamily(widget.params).notifier,
          next,
        );
        onErrorShown();
      }
  ...
  Widget buildScreen(
    ...
    NotifierProvider<VM, S> Function(P params) provider,
  );
}
