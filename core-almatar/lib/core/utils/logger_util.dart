...
class MyLogger {
  ...
  static void _log(String level, String message, [Object? error, StackTrace? stackTrace]) {
    ...
    // Always write to developer log for structured logging and also print to
    // console (debugPrint) so messages show up reliably in IDE consoles.
    if (error != null) {
      developer.log(message, name: level, time: timestamp, error: error);
      if (kReleaseMode) debugPrint('[$level] $message - $error');
    } else if (stackTrace != null) {
      developer.log(message, name: level, time: timestamp, stackTrace: stackTrace);
      if (kReleaseMode) debugPrint('[$level] $message - $stackTrace');
    } else {
      developer.log(message, name: level, time: timestamp);
      if (kReleaseMode) debugPrint('[$level] $message');
    }
  }
}