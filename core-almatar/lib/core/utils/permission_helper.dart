...
class PermissionHelper {
  ...
  /// Handle contacts permission when permanent deny
  static Future<void> handleContactsPermissionWhenDenied({
    required BuildContext context,
    required String title,
    required String message,
  }) async {
    bool isPermanentlyDenied =  await Permission.contacts.status.isPermanentlyDenied;
    bool isRestricted = await Permission.contacts.status.isRestricted;

    MyLogger.info("isPermanentlyDenied ===> $isPermanentlyDenied");
    MyLogger.info("isRestricted ===> $isRestricted");

    if (isPermanentlyDenied || isRestricted) {
      await _showPermissionDialog(
        context,
        title: 'Contacts Permission Required',
        message: 'Contacts permission is required for this feature.',
      );
    }
  }
