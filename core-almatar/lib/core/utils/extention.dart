...
import 'package:core/core/local/secure_storage_keys.dart'
    show SecureStorageKeys;
import 'package:core/core/utils/click_debouncer.dart';
...
extension WidgetDirectionalPaddingExtension on Widget {
  ...
  Widget paddingDirectionalAll(BuildContext context, double value) => Padding(
    padding: context.scaledUniformDirectionalPadding(value),
    child: this,
  );

  /// Apply directional symmetric padding (RTL-aware)
  Widget paddingDirectionalSymmetric(
  ...
  Widget paddingHorizontalDirectional(
    BuildContext context, {
    double left = 0,
    double right = 0,
    double top = 0,
    ...
    padding: context.scaledHorizontalDirectionalPadding(
      left: left,
      right: right,
      top: top,
      bottom: bottom,
...
extension WidgetDirectionalMarginExtension on Widget {
  ...
  Widget marginDirectionalAll(BuildContext context, double value) => Container(
    margin: context.scaledUniformDirectionalPadding(value),
    child: this,
  );

  /// Apply directional symmetric margin (RTL-aware)
  Widget marginDirectionalSymmetric(
    BuildContext context, {
    double horizontal = 0,
    double vertical = 0,
  }) => Container(
    margin: context.scaledSymmetricDirectionalPadding(
      horizontal: horizontal,
      vertical: vertical,
    ),
    child: this,
  ...
  Widget marginHorizontalDirectional(
    BuildContext context, {
    double left = 0,
    double right = 0,
    double top = 0,
    ...
    margin: context.scaledHorizontalDirectionalPadding(
      left: left,
      right: right,
      top: top,
      bottom: bottom,
...
///
/// Provides convenient tap handlers with built-in protection against multiple rapid clicks.
/// All tap handlers are automatically debounced to prevent duplicate actions.
extension Clickable on Widget {
  /// Wrap widget with tap handler and automatic debouncing
  ///
  /// [onTap] - Callback to execute on tap (automatically debounced)
  /// [borderRadius] - Optional border radius for ripple effect
  /// [enableDebounce] - Set to false to disable debouncing (default: true)
  /// [debounceDuration] - Time between allowed taps (default: 500ms)
  Widget onTap({
    required VoidCallback? onTap,
    BorderRadius? borderRadius,
    bool enableDebounce = true,
    Duration debounceDuration = ClickDebouncer.defaultDuration,
  }) {
    return InkWell(
      onTap: onTap == null
          ? null
          : enableDebounce
          ? () => ClickDebouncer().run(onTap, duration: debounceDuration)
          : onTap,
      borderRadius: borderRadius,
  ...
  /// Wrap widget with tap handler that requires login (with automatic debouncing)
  ///
  /// [onTap] - Callback to execute on tap (automatically debounced)
  /// [borderRadius] - Optional border radius for ripple effect
  /// [isLoginRequired] - Whether to check login status before executing (default: true)
  /// [enableDebounce] - Set to false to disable debouncing (default: true)
  /// [debounceDuration] - Time between allowed taps (default: 500ms)
  Widget onConsumerTap({
    required VoidCallback onTap,
    BorderRadius? borderRadius,
    ...
    return Consumer(
      builder: (context, ref, child) {
        return InkWell(
          onTap: () async {
            // Wrap the entire logic in debouncer to prevent multiple simultaneous checks
            ClickDebouncer().run(() async {
              if (isLoginRequired) {
                bool isLogin = bool.parse(
                  await ref
                          .read(localRepositoryProvider)
                          .getData(SecureStorageKeys.kIsLogin) ??
                      "false",
                );
                if (!isLogin) {
              ...
              onTap();
            }, duration: enableDebounce ? debounceDuration : Duration.zero);
          },
          borderRadius: borderRadius,
          splashColor: Colors.transparent,
          highlightColor: Colors.transparent,
...
extension ContainerExtensions on BuildContext {
  ...
  Widget styledProductContainer({
    ...
    return Container(
      ...
      padding: scaledSymmetricDirectionalPadding(
