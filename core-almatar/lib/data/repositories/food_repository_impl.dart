...
class FoodRepositoryImpl implements FoodRepository {
  ...
  @override
  Future<Either<String, FoodOrderTrackingResponse>> foodOrderTracking(String fullUrl, List<String> headersList) async {
    try {
      // Parse the headers list into individual values
      ...
      print("url in repo: $fullUrl");
      print("parsed custom headers: $customHeaders");

      // Use Dio instance directly
      final response = await Dio().get(fullUrl, options: Options(headers: customHeaders));

      // Parse the response manually
      final trackingResponse = FoodOrderTrackingResponse.fromJson(response.data);
  ...
  @override
  Future<Either<String, CommonResponse<FoodOrderInvoiceResponse>>> foodDeliveryOrdersInvoice(int orderId) async {
    try {
      final posts = await apiService.foodDeliveryOrdersInvoice(orderId);
      return Right(posts);
    } on DioException catch (dioError) {
      // Interceptor already parsed the error message
      return Left(dioError.message ?? 'An error occurred');
...
// utils/header_parser.dart
class HeaderParser {
  static Map<String, String> parseHeaders(List<String> headersList) {
