...
class RideHailingRepositoryImpl implements RideHailingRepository {
  ...
  @override
  Future<Either<String, CommonResponse<ChatData>>> chatHistory(int bookingId, int page, int limit) async {
    try {
      final response = await apiService.chatHistory(bookingId, page, limit);
      return Right(response);
    } on DioException catch (dioError) {
      return Left(dioError.message ?? 'An error occurred');
    } catch (e) {
