...
class GroceryRepositoryImpl implements GroceryRepository {
  ...
  @override
  Future<Either<String, CommonResponse<List<GrocerySubCategoryResponse>>>> groceryProductList(Map<String, dynamic> params) async {
    try {
  ...
  @override
  Future<Either<String, GroceryOrdersTrackingResponse>> groceryOrderTracking(String fullUrl,String token) async {
    try {
      final posts = await apiService.groceryOrderTracking(fullUrl,token);
      return Right(posts);
    } on DioException catch (dioError) {
      // Interceptor already parsed the error message
      return Left(dioError.message ?? 'An error occurred');
