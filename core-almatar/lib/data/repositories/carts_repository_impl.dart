...
class CartsRepositoryImpl implements CartsRepository {
  ...
  @override
  Future<Either<String, CommonResponse<InitiateTamaraPayment>>> doInitiateTamaraPayment(Map<String, dynamic> params) async {
    try {
      final posts = await apiService.doInitiateTamaraPayment(params);
      return Right(posts);
    } on DioException catch (dioError) {
      /// Interceptor already parsed the error message
      return Left(dioError.message ?? 'An error occurred');
  ...
  @override
  Future<Either<String, CommonResponse>> deleteCart(Map<String, dynamic> params) async {
    try {
      final posts = await apiService.deleteFoodCart(params);
      return Right(posts);
    } on DioException catch (dioError) {
      /// Interceptor already parsed the error message
      return Left(dioError.message ?? 'An error occurred');
